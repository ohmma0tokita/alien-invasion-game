...
def write(msg, z=30):
    # write message
    font = pygame.font.SysFont('Comic Sans MS', z)
...
def check_database(STATS):
    ...
    if i:
        time.sleep(0.5)
        print('\tGood to see you again ' + STATS.name.title() + '!')
        time.sleep(0.5)
        return i, data

def update_database(STATS, data, i):
    if i: 
        if data["record"]> STATS.points > data["players"][i][STATS.points]: # if new score is better than old take it
            data["players"][i][STATS.name] = STATS.points
            return "u re getting better"
    
    else:
        with open('Alien_Invasion.json', 'r+') as file:
            data["players"].append({STATS.name: STATS.points})
            file.seek(0)
            json.dump(data, file, indent = 4)
    
    if STATS.points > data["record"]: # checks for new record
        data["record"] = STATS.points
        data["holder"] = STATS.name
